...
%Maaz ur Rehman, Jamilu Sabi'u, Muhammad Sohaib and Abdullah Shah\cite{rehman2023projection}proposes a projection-based derivative-free DFP method for convex-constrained monotone nonlinear equations. An optimal scaling parameter is derived by minimizing the condition number, leading to a memoryless search direction that avoids Jacobian computation. Global convergence is proved under monotonicity. Numerical results show improved iterations and CPU time over existing methods. Image restoration applications demonstrate higher computational efficiency while maintaining reconstruction quality.